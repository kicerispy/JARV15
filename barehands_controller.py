...
class BarehandsController:
    """HTTP adapter for a running barehands server plus its ring state file."""
    ...
    def _failure(tool: str, error: str, retryable: bool = True) -> ToolResult:
        return ToolResult(success=False, tool=tool, error=error, retryable=retryable)
    ...
    def set_state(self, state: str) -> ToolResult:
        """Write the documented state/state file; barehands reads it via /orb."""
        ...
        if self.state_dir is None:
            return self._failure(
                tool,
                "Barehands state directory is not configured. Set BAREHANDS_DIR.",
                retryable=False,
            )
        try:
            self.state_dir.mkdir(parents=True, exist_ok=True)
            (self.state_dir / "state").write_text(state, encoding="utf-8")
            return ToolResult(success=True, tool=tool, data={"state": state})
        except Exception as exc:
            return self._failure(tool, f"Barehands state update failed: {exc}", retryable=False)
    ...
    def board_command(self, action: str, **payload: Any) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(success=True, tool=tool, data=command)
    ...
    def add_image(self, src: str, title: str = "", body: str = "") -> ToolResult:
        return self.board_command("add_img", src=str(src), title=str(title), body=str(body))
