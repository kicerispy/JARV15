...
AVAILABLE_TOOLS: Dict[str, str] = {
    ...
    "jarvis_quickcheck": "Run a fast deterministic JARVIS readiness check covering runtime, resources, memory, healing, and learned autonomy.",
    "resource_status": "Report CPU, RAM, disk, and available CPU cores.",
    ...
    "service_status": "Probe local Ollama and n8n dependencies. Argument is ollama, n8n, or all.",
    "dependency_status": "Check declared Python requirements against the active environment.",
    "code_index_rebuild": "Build or rebuild the local FTS5 code index for fast source discovery.",
    "code_index_status": "Report local code-index status.",
    "healing_hints": "Inspect learned failure/recovery patterns. Argument is JSON with optional tool, category, error, and limit.",
    "tool_health": "Show per-tool reliability, latency, failures, and circuit-breaker state.",
    "memory_remember": "Store a local JARVIS memory item. Argument is JSON with text, kind, and optional tags.",
    "memory_recall": "Recall relevant local JARVIS memory. Argument is JSON with query, limit, and optional kind.",
    "memory_forget": "Forget matching local JARVIS memory. Argument is a query or JSON object.",
    "ollama_models": "List locally available Ollama models and configured models that are missing.",
    "healing_history": "Show recent sanitized self-healing events. Argument is an optional integer limit.",
    "tool_reset": "Reset a tool's resilience/circuit state. Argument is the tool name or JSON with tool.",
    "memory_status": "Show local memory counts and storage information.",
    "jarvis_capabilities": "List the active JARVIS tool inventory and capability categories.",
    ...
    "tool_contract_audit": "Audit planner-visible tools against the canonical registry and report capability drift.",
    "integration_health": "Run the read-only integration health matrix for JARVIS and optional subsystems.",
    ...
    "hindsight_status": "Check the optional local Hindsight long-term memory service.",
    "hindsight_remember": "Persist durable JARVIS memory in Hindsight. Argument is JSON with content and optional document_id, context, tags.",
    "hindsight_recall": "Recall durable Hindsight memories relevant to a query. Argument is JSON with query, limit, and optional budget.",
    "hindsight_reflect": "Use Hindsight reflection over durable memory to answer a continuity question. Argument is JSON with query and optional budget/max_tokens.",
    ...
    "roblox_mcp_setup": "Install or repair the Roblox Studio MCP bridge after an explicit setup request.",
    "screen_memory_setup": "Install or configure the local Screenpipe bridge after an explicit setup request.",
