...
def deterministic_route(user_request, active_context=None):
    ...
    # n8n workflow discovery and mutations must reach the MCP-aware planner.
    # Never let generic browser "find" or DOM shortcuts hijack them.
    try:
        from planner import is_n8n_workflow_request
        if is_n8n_workflow_request(text):
            print("JARVIS: n8n workflow request routed to MCP planner.")
            return None
    except Exception:
        pass
