...
def locate_youtube_thumbnail(
    ...
    # --------------------------------------------------
    # Deterministic YouTube thumbnail geometry.
    #
    ...
    #
    # Do not ask the vision model to invent thumbnail
    # coordinates. It has produced unreliable boxes.
    #
    # YouTube search thumbnails are on the left side of
    # the result row and are approximately 16:9.
    # --------------------------------------------------

    thumbnail_left = 0

    # Keep the clickable region conservative. The right
    # edge stays well before title/channel/action controls.
    thumbnail_width = int(
        round(
            candidate_width
            *
    ...
    # Construct a genuine 16:9 rectangle.
    ...
    # If the row is shorter than the calculated thumbnail,
    # shrink the thumbnail while preserving 16:9.
    max_height = int(
        round(
            candidate_height
            *
    ...
    # Center the 16:9 thumbnail vertically inside the
    ...
    if (
        ...
        thumbnail_top < 0
        or
        thumbnail_right > candidate_width
        or
        thumbnail_bottom > candidate_height
    ):

        logging.info(
            f"YouTube deterministic thumbnail "
            f"candidate {candidate_index}: "
            "region escaped candidate bounds."
        )
    ...
    # Allow only a very small tolerance around 16:9.
    if (
        aspect_ratio < 1.70
        or
        aspect_ratio > 1.90
