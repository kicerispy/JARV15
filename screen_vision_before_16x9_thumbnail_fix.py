...
def locate_youtube_thumbnail(
    ...
    # --------------------------------------------------
    # The candidate has already been independently verified
    # as an organic YouTube result.
    #
    # Do NOT rely on the vision model to produce thumbnail
    # geometry. The model has repeatedly returned either:
    #
    #   [0, 0, 1000, 1000]
    #
    # or malformed coordinate systems.
    #
    # YouTube search-result rows place the thumbnail on the
    # left side of the row. Use a conservative deterministic
    # region instead.
    #
    # We intentionally leave substantial room on the right
    # so title/channel/action controls cannot become the click
    # target.
    # --------------------------------------------------
    ...
    if (
        ...
        logging.info(
            f"YouTube deterministic thumbnail "
            f"candidate {candidate_index}: "
            "region too narrow: "
    ...
    # --------------------------------------------------
    # Reject obviously wrong geometry.
    #
    # A normal YouTube thumbnail is approximately 16:9.
    # This broad range tolerates row scaling while rejecting
    # square/tall areas and whole-row selections.
    # --------------------------------------------------
    ...
    if (
        ...
        logging.info(
            f"YouTube deterministic thumbnail "
            f"candidate {candidate_index}: "
            f"unsafe aspect ratio {aspect_ratio:.2f}"
        )
